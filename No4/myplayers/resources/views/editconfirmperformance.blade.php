...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/editconfirmperformance.css') }}">
</head>
<body>
    <h1>成績編集確認</h1>
    <table>
    ...
    <h3>上記の情報で更新しますか？</h3>
        ...
        <form action="{{ route("performances.modify") }}" method="post">
            ...
            <button type="submit" name="edit" value="true">戻る</button>
        </form>
    
        {{-- 更新処理 --}}
        <form action="{{ route("performances.update") }}" method="post">
            @csrf
            <input type="hidden" name="id" value={{ $inputs['id'] }}>
            <input type="hidden" name="player_id" value={{ $inputs['player_id'] }}>
            <input type="hidden" name="game_date" value={{ $inputs['game_date'] }}>
